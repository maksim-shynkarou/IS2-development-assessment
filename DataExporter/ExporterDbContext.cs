...
namespace DataExporter
{
    public class ExporterDbContext : DbContext
    {
        ...
        {
            modelBuilder.Entity<Policy>().HasMany(x => x.Notes).WithOne(x => x.Policy).HasForeignKey(x => x.PolicyId);
            ...
            modelBuilder.Entity<Note>().HasData(new Note() { Id = 1, Text = "Some note 1", PolicyId = 1 },
                new Note() { Id = 2, Text = "Some note 2", PolicyId = 1 },
                new Note() { Id = 3, Text = "Some note 3", PolicyId = 2 },
                new Note() { Id = 4, Text = "Some note 1", PolicyId = 3 });
